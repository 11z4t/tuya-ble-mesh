...
**Symptoms:**
- Error during device setup: "Pairing failed"
- Device not responding to provisioning commands
- Timeout during provisioning process

**Solutions:**
1. **Check device is unpaired:**
   - Perform factory reset (power cycle 5-10 times)
   - Verify device advertises as `out_of_mesh*`
   - Device should NOT already be in a mesh network
2. **Verify protocol type:**
   - **SIG Mesh devices** (Smart Plug S17): Look for GATT services `0x1827` or `0x1828`
   - **Telink Mesh devices** (LED Driver): Look for service UUID `fe07`
   - Use correct pairing flow for your device type
3. **Reduce distance:**
   - Move device within 2 meters of Bluetooth adapter/proxy during pairing
   - Strong signal (RSSI > -60 dBm) improves pairing reliability
4. **Check for interference:**
   - Disable other BLE apps/scans during pairing
   - Pause other integrations temporarily
   - Ensure no other device is trying to pair simultaneously
5. **Verify bridge connectivity (Telink Mesh only):**
   - If using bridge daemon, ensure it's running: `systemctl status tuya-bridge`
   - Check bridge logs for connection errors
   - Test bridge API: `curl http://<bridge-ip>:8787/status`
6. **Try direct connection first:**
   - For SIG Mesh devices, try pairing without ESPHome proxy
   - Use direct Home Assistant Bluetooth (if available)
   - Add ESPHome proxy after successful pairing

### Problem: Bridge Auto-Detection Not Working

**Symptoms:**
- Integration doesn't detect bridge daemon automatically
- "No bridge found" error during setup
- Telink Mesh device not connecting

**Solutions:**
1. **Verify bridge daemon is running:**
   ```bash
   # On the Raspberry Pi running the bridge
   ps aux | grep ble_mesh_daemon
   # Should show: python scripts/ble_mesh_daemon.py --host 0.0.0.0 --port 8787
   ```
2. **Check network connectivity:**
   ```bash
   # From Home Assistant host
   curl http://<bridge-ip>:8787/status
   # Should return: {"status": "ok", "version": "..."}
   ```
3. **Verify firewall rules:**
   - Ensure port `8787` is open on bridge host
   - Check UFW/iptables: `sudo ufw status` or `sudo iptables -L`
   - Allow incoming TCP on port 8787
4. **Check bridge discovery broadcast:**
   - Bridge announces itself via mDNS (Avahi/Bonjour)
   - Verify mDNS is working: `avahi-browse -a | grep tuya`
   - If mDNS is disabled, use manual IP entry instead
5. **Manual bridge configuration:**
   - During setup, select "Manual Bridge Configuration"
   - Enter bridge IP address (e.g., `192.168.1.100`)
   - Enter bridge port (default `8787`)
6. **Check bridge logs:**
   ```bash
   # On bridge host
   journalctl -u tuya-bridge -f
   # Look for connection errors or Bluetooth adapter issues
   ```

### Problem: ESPHome Proxy Not Detecting Devices

**Symptoms:**
- ESPHome BLE proxy is online but devices don't appear
- "No devices found" in Home Assistant Bluetooth scanner
- SIG Mesh devices not discovered via proxy

**Solutions:**
1. **Verify ESPHome proxy configuration:**
   ```yaml
   # In ESPHome config, ensure:
   bluetooth_proxy:
     active: true
   ```
   - **Not**: `esp32_ble_tracker` (different component)
2. **Check proxy is connected:**
   - Go to **Settings** → **Devices & Services** → **ESPHome**
   - Verify proxy shows as "Online"
   - Check "Bluetooth" indicator is green
3. **Verify SIG Mesh device compatibility:**
   - ESPHome proxies **only work with SIG Mesh** devices
   - Telink Mesh devices require bridge daemon
   - Check device GATT services: `0x1827`/`0x1828` = SIG Mesh
4. **Reduce distance:**
   - Place ESPHome proxy within 5 meters of target device
   - Check ESP32 antenna is not obstructed
5. **Check ESP32 Bluetooth is enabled:**
   - ESPHome logs should show: "Bluetooth Proxy started"
   - If missing, reflash ESP32 with correct config
6. **Verify Home Assistant Bluetooth integration:**
   - Go to **Settings** → **Devices & Services** → **Bluetooth**
   - Ensure HA Bluetooth integration is enabled
   - ESPHome proxies extend HA Bluetooth, they don't replace it
7. **Restart proxy:**
   - Reboot the ESP32 device
   - Wait 30 seconds for full initialization
   - Check ESPHome logs for errors

### Problem: Vendor ID Trial-and-Error

**Symptoms:**
- Device pairs successfully but commands don't work
- Light/switch doesn't respond to on/off or brightness commands
- Logs show "command timeout" or "no response"

**Background:**
Different Tuya/Telink device manufacturers use different **vendor IDs** in the mesh protocol. The vendor ID must match for encryption/decryption to work correctly.

**Known Vendor IDs:**
| Brand | Vendor ID | Hex Bytes (LE) |
|-------|-----------|----------------|
| Malmbergs BT Smart | `0x1001` | `01 10` |
| AwoX / Eglo | `0x0160` | `60 01` |
| Dimond / retsimx | `0x0211` | `11 02` |

**Trial-and-Error Procedure:**

1. **Start with default (`0x1001`):**
   - During setup, use `0x1001` (Malmbergs default)
   - Try controlling the device (on/off)

2. **If commands fail, try `0x0160` (AwoX):**
   - Go to **Settings** → **Devices & Services** → **Tuya BLE Mesh**
   - Click on the device
   - Click **Configure**
   - Change **Vendor ID** to `0x0160`
   - Click **Submit**
   - Try controlling again

3. **If still failing, try `0x0211` (Dimond):**
   - Repeat step 2 with `0x0211`
...
4. **Enable debug logging to verify:**
   ...
   data:
     level: debug
   ```
   - Look for "Command sent successfully" vs. "Timeout" in logs
   - Correct vendor ID will show successful command acknowledgment

5. **Find vendor ID from packet capture (advanced):**
   - Pair device with official Tuya/Malmbergs app
   - Enable Android HCI snoop log (Developer Options)
   - Capture BLE traffic while controlling device
   - Open in Wireshark, filter for GATT writes to `0x1912`
   - Vendor ID is bytes [3:5] in little-endian (e.g., `01 10` = `0x1001`)

6. **Report findings:**
   - Once you find the working vendor ID, please report it!
   - Open issue on GitHub with device model + vendor ID
   - Helps future users with the same device

**Why This Happens:**
- Tuya licenses its protocol to many manufacturers
- Each manufacturer gets a unique vendor ID
- The encryption key derivation uses the vendor ID
- Wrong vendor ID = wrong encryption key = commands ignored
