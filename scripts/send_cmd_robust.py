...
def _make_cmd(
    ...
    payload = (struct.pack("<H", dest) + struct.pack("B", opcode) + vendor_id + params).ljust(
        15, b"\x00"
    )
...
async def main() -> None:
    ...
    print(f"\n{'=' * 60}")
    print("  Robust Command Test")
    print(f"  Target: {mac}")
    print("  Creds: out_of_mesh / 123456 (confirmed)")
    print(f"{'=' * 60}\n")
    ...
    try:
        ...
        tests = [
            ...
            ("Power ON, dest=1, vendor=0x1102, WC", 1, 0xD0, bytes([0x11, 0x02]), b"\x01", False),
        ]
        ...
        for i, (label, dest, opcode, vendor_id, params, use_resp) in enumerate(tests):
            ...
            try:
                ...
                await client.write_gatt_char(TELINK_CHAR_COMMAND, cmd, response=use_resp)
